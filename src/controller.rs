use crate::{ble, ble_notify_parser, predefined, true_gear_message::{self}};
use std::{error::Error};
...
impl TrueGearBLEController {
    ...
    {
        let _ = self.send_ble_messages(&predefined::on_connected_message()).await;
    }
    ...
    pub async fn send_ble_messages(&mut self, messages: &[true_gear_message::Message]) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    pub async fn send_ble_message(&mut self, message: true_gear_message::Message) -> Result<(), Box<dyn Error + Send + Sync>> {
